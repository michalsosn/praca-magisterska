...
\begin{itemize}
    \item 10 wypowiedzi miało stałą treść dla wszystkich uczestników i~we wszystkich sesjach. Stanowią one podstawę
        ...
        z~innego zbioru \shortcut{TIMIT}\cite{darpaTimitAcoustic}.
...
rejestracyjnych (\foreign{enrollment}) i~zestaw nagrań weryfikacyjnych. (\foreign{trial}) Problemy te to:
\begin{enumerate}
...
\techname{CMUDict}\cite{TheCMUPronouncing} to zbiór zawierający zapisy fonetyczne ponad 134000 angielskich słów.
...
oraz \techname{TensorFlow}, zawierających implementacje metod, które wykorzystano w~pracy.
Poza tym bezproblemowo integruje się środowiskiem ze \techname{Jupyter}, które jest bardzo wygodne przy
...
W~pracy \techname{TensorFlowa} jest wykorzystany do wczytania modelu stworzonego przez organizację \techname{Mozilla}.
Model ten powstał w~oparciu o~pracę \techname{DeepSpeech}\cite{deepSpeechScaling},
opisującą komercyjny model do rozpoznawania mowy rozwinięty przez \techname{Baidu}.
Ten model jest wczytywany w~sesji \techname{TensorFlowa} i~używany do dopasowania ramek do klas.
...
Poza tym warto wspomnieć, że wykorzystano \techname{gita} do wersjonowania pracy, zarówno \foreign{notebooków}, pakietów jak i~tekstu tego dokumentu. Do zarządzania pakietami wykorzystano \techname{pip}, ale być może lepszym wyborem są nowsze narzędzia jak \techname{conda} lub \techname{pipenv}.
