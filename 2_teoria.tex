...
gdzie $\alpha_k = \frac{N_k}{N_k + r}$, $\bar{\mu_k} = \frac{F_k}{N_k}$, $N_k = \sum_{x_i \in X} P(\theta_k | x_i)$, $F_k = \sum_{x_i \in X} P(\theta_k | x_i) x_i$
...
Dlatego wylicza się pole pod krzywą \shortcut{ROC}, tzw. \shortcut{AUC}. (\textbf{\foreign{area under the curve}})
...
Alternatywą do \shortcut{AUC} jest \shortcut{EER} (\textbf{\foreign{equal error rate}}). Wyznacza się je
